...
const Artists = () => {
  ...
  const getuser = async (start,tsort)=>{
    ...
    setSearch('');
    setSdate('');
    const data = await fetch(`http://localhost:8080/${tsort}/${admin}/${start}/${pagesize}`,{
      method:"GET",
      headers:{
        "Content-Type":'application/json'
      }
    ...
    <div>
      ...
      <div className="t-box">
        <div className="filter" id="filter">
          ...
          <div className="hid-sec" id="hid-sec" style={{display:'flex',alignItems:'center'}}>
            <div>
            <button onClick={sortbyName} style={{backgroundColor:`${tsort=="name"?"rgb(255,255,255,.5)":"unset"}`}}>SortbyName </button>
            <button onClick={sortbyTime} style={{backgroundColor:`${tsort=="time"?"rgb(255,255,255,.5)":"unset"}`}}>Sortbytime </button>
            <button onClick={sortbyTimed} style={{backgroundColor:`${tsort=="timed"?"rgb(255,255,255,.5)":"unset"}`}}>Sortbytime-dsc </button>
            </div>
          

            <input placeholder="seach by date" style={{backgroundColor:'transparent',color:'white'}} type="text" 
            value={sdate}
            onChange={(e)=>{
              e.preventDefault();
            ...
            <input style={{backgroundColor:'transparent',color:'white'}} value={search} onChange={(e)=>{
              e.preventDefault();
            ...
            <button onClick={()=>{
              getuser(0,"get");
            }} style={{marginLeft:'50px'}}>All</button>
        ...
        <div style={{display:`${issearch?"none":"flex"}`,justifyContent:'center',alignItems:'center'}}>
          <h2 style={{color:'white'}}>Pages: </h2>
          {
            arr.map((e)=>{
              return <button  onClick={(f)=>{
                ...
                }} style={{margin:'5px',padding:'5px',background:`${e*pagesize==start?"white":"rgb(255,255,255,.3)"}`}} key={e}>{e+1}</button>
            })
          }
        </div>
      </div>
